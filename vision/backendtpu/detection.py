...
class Detection:
    ...
    def draw_detections(self, img, objs):
        ...
        - objs (list): List of detected objects.
        ...
        for obj in objs:
            ...
            draw.text((bbox.xmin, bbox.ymin), f"{label} ({obj.score:.2f})", fill="red")
        return img
            ...
            with conn:
                try:
                    ...
                    if len(data) == size:
                        ...
                        # Process the image (detection)
                        objs = model_detection.detect_from_image(img)

                        # Draw detections on the image
                        processed_img = model_detection.draw_detections(img, objs)
                        ...
                        # Send the size of the processed image data
                        conn.sendall(struct.pack(">I", len(processed_img_data)))
                        # Send the processed image data
                        conn.sendall(processed_img_data)
                        print("Processed image sent back to the client")
                    else:
                        print(f"Incomplete data received. Expected {size} bytes, got {len(data)} bytes.")
